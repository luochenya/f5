            ...
            <div class="from">
              <div class="item-input" :class="{ checked: errStatus7 }">
                <label :prop="prop.oldpassword">
                  ...
                  <input
                    ...
                    @blur.prevent="changeIpt('oldpassword')"
                  />
                </label>
                <div :class="{ error: errStatus7 }">
                  <span></span>{{ errMessage7 }}
                </div>
              </div>
              <div class="item-input" :class="{ checked: errStatus8 }">
                <label :prop="prop.password">
                  ...
                  <input
                    type="text"
                    placeholder="密碼長度至少6位數"
                    v-model="form.password"
                    @blur.prevent="changeIpt('password')"
                  />
                </label>
                <div :class="{ error: errStatus8 }">
                  <span></span>{{ errMessage8 }}
                </div>
              </div>
              <div class="item-input" :class="{ checked: errStatus9 }">
                <label :prop="prop.repassword">
                  ...
                  <input
                    ...
                    @blur.prevent="changeIpt('repassword')"
                  />
                </label>
                <div :class="{ error: errStatus9 }">
                  <span></span>{{ errMessage9 }}
                </div>
              </div>
            </div>
          </form>
  ...
  methods: {
    ...
    submit() {
      ...
      if (
        ...
        updatePassword(this.form, { headers: { token: this.token } }).then(
          res => {
...
.resetas-psword-modal {
  ...
  .modal-dialog {
    ...
    background: url("./../assets/imgs/resetas-psword-modal.png") no-repeat top
    ...
    .modal-body {
      .from {
        ...
        .checked {
          border: 0.1rem solid rgba(235, 54, 44, 1) !important;
        }
      }
    }
    .modal-footer {
