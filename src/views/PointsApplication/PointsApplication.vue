<template>
  <div class="PointsApplication">
      ...
      <div class="centent">
        <div class="item-left">
          ...
          <div class="bottom-msg">
            <span @click="$router.push({ path: '/memberCenter' })"
              >會員資料修改</span
            >
            <div></div>
            <span @click="$router.push({ path: '/myPublishedArticles' })"
              >我發表的文章</span
            >
            <div></div>
            <span class="bottom-msg-active"
              >點數申請<i class="icon-9"></i
            ></span>
            <div></div>
            <span @click="$router.push({ path: '/PointsRecord' })"
              >點數記錄</span
            >
            <div></div>
            <span @click="$router.push({ path: '/MallOrder' })"
              >商城訂單查詢</span
            >
            <div></div>
            <span @click="$router.push({ path: '/notificationCenter' })"
              >通知中心</span
            >
          </div>
        </div>
        <div class="item-r">
          <div class="title">
            <div class="title-l">
              <h1>
                點數申請
                <em @click="ApplicationInstructions()">查看申請說明</em>
              </h1>
          ...
          <div class="taba">
            ...
            <div>
              <div class="active">
                <div class="prompt" v-if="ArticlesList.length <= 0">
                  暫無申請記錄
                </div>
                <ul v-else>
                  <li
                  ...
                  >
                    ...
                    <div class="right-box">
                      <p v-if="item.state == 0" class="right_box_Processing">
                        處理中...
                      </p>
                      <p v-if="item.state == 1" class="right_box_By">通過</p>
                      <p v-if="item.state == 2" class="right_box_DidNotPass">
                        不通過
                      </p>
                    </div>
                  </li>
    ...
    >
      <template slot="body">
        <h3>申請說明</h3>
        <em>{{ ApplicationInstructionsContent }}</em>
      </template>
    </modal>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    // 獲取申请说明
    _getPointsApplyExplain() {
      getPointsApplyExplain({ headers: { token: this.token } }).then(res => {
        if (res.data.code == "200") {
          this.ApplicationInstructionsContent = res.data.data.content;
        }
      });
    },
    // 點數申請
        ...
        .btn {
          ...
          span {
            @include bgImg(
              1.2rem,
              1.2rem,
              "../../assets/imgs/icon/icon-center.png"
            );
            margin-right: 0.6rem;
          }
        }
    ...
    .item-r {
      ...
      .taba {
        ...
        .tabas {
          ...
          .active {
            border-bottom: 0.2rem solid rgba(37, 36, 39, 1);
            a {
      ...
      .active {
        ...
        ul > li {
          @include flex();
          padding: 3.2rem 0;
          border-bottom: 0.1rem solid rgba(247, 248, 250, 1);
          ...
          .left-box {
            ...
            span {
              @include bgImg(
                5rem,
                5rem,
                "../../assets/imgs/icon/icon-articles-2.png"
              );
            }
            .bg-img {
              @include bgImg(
                5rem,
                5rem,
                "../../assets/imgs/icon/icon-articles-1.png"
              );
            }
            .text {
              h6 {
