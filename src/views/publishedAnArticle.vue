    ...
    <div class="tabs">
      <div class="container">
        <el-breadcrumb separator-class="icon-3">
          <el-breadcrumb-item
            ><a @click="$router.push('/')">首頁</a></el-breadcrumb-item
          >
          <el-breadcrumb-item
            ><a @click="$router.push('myPublishedArticles')"
              >我的文章</a
            ></el-breadcrumb-item
          >
          <el-breadcrumb-item :class="{ checked: $route.meta.title }">{{
            $route.meta.title
          }}</el-breadcrumb-item>
        </el-breadcrumb>
      </div>
            ...
            <div class="card">
              <ul>
                <li
                  ...
                  :key="index"
                  :class="{ on: typeIndex == item.id }"
                  @click="check(index, item.id)"
                >
                  {{ item.class_name }}
                </li>
              </ul>
...
import {
  uploadPicture,
  systemGetArticleClass,
  setInsertArticle,
  setInsertDraft,
  getFeaturedArticlesRead,
  setInsertDrafts,
  delInsertArticle
} from "./../api/request";
...
export default {
  ...
  created: function() {
    let that = this;
    that.upLoadUrl =
      process.env.VUE_APP_BASEURL + "/System/upload_picture_text";
    ...
    addfrom(type) {
      ...
      if (type === 1) {
        ...
        if (this.article_id) {
          ...
          setInsertDrafts(this.form, { headers: { token: this.token } }).then(
            res => {
              this.isShowLoading = false;
              if (res.data.code !== "200") {
          ...
          setInsertDraft(this.form, { headers: { token: this.token } }).then(
            res => {
              this.isShowLoading = false;
              if (res.data.code !== "200") {
                this.$message.error(res.data.msg);
        ...
        this.form.type = "web";
        setInsertArticle(this.form, { headers: { token: this.token } }).then(
          res => {
            this.isShowLoading = false;
            if (res.data.code !== "200") {
              this.$message.error(res.data.msg);
    ...
    .main {
      ...
      .item-l {
        ...
        .card {
          ul {
            ...
            flex-wrap: wrap;
            li {
              padding: 0.6rem 1rem 0.7rem 2.2rem;
              margin-right: 1.4rem;
      ...
      .item-r {
        ...
        .card {
          ...
          background: rgba(245, 246, 247, 1);
          border-radius: 0.6rem;
          border: 0.1rem solid rgba(239, 239, 239, 1);
